...
@section('title', 'Edit Supply Request - DARTS')
@section('page-title', 'Edit Supply Request')
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-100">
            <div class="flex items-center space-x-2 text-sm text-gray-500">
                ...
                <a href="{{ route('supply-requests.show', $supplyRequest->request_id) }}" class="hover:text-emerald-600">#{{ $supplyRequest->request_id }}</a>
                <i class="fa-solid fa-chevron-right text-xs"></i>
                <span class="text-gray-900">Edit</span>
            </div>
        </div>

        <form method="POST" action="{{ route('supply-requests.update', $supplyRequest->request_id) }}" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="department_unit" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 @error('department_unit') border-red-500 @enderror">
                            @foreach(['Academic', 'Administration', 'Finance', 'HR', 'IT', 'Logistics', 'Supply Office'] as $dept)
                                <option value="{{ $dept }}" {{ old('department_unit', $supplyRequest->department_unit) == $dept ? 'selected' : '' }}>{{ $dept }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" name="date_needed" value="{{ old('date_needed', $supplyRequest->date_needed ? \Carbon\Carbon::parse($supplyRequest->date_needed)->format('Y-m-d') : '') }}"
                    ...
                    <div>
                        ...
                        <select name="purpose" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 @error('purpose') border-red-500 @enderror">
                            @foreach(['Classroom Use', 'Office Use', 'Event', 'Repair & Maintenance', 'Stock Replenishment', 'Other'] as $opt)
                                <option value="{{ $opt }}" {{ old('purpose', $supplyRequest->purpose) == $opt ? 'selected' : '' }}>{{ $opt }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="sales_type" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                            ...
                            <option value="Direct" {{ old('sales_type', $supplyRequest->sales_type) == 'Direct' ? 'selected' : '' }}>Direct</option>
                            <option value="Consignment" {{ old('sales_type', $supplyRequest->sales_type) == 'Consignment' ? 'selected' : '' }}>Consignment</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="request_type" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                            <option value="">Select Type</option>
                            <option value="New" {{ old('request_type', $supplyRequest->request_type) == 'New' ? 'selected' : '' }}>New</option>
                            <option value="Replacement" {{ old('request_type', $supplyRequest->request_type) == 'Replacement' ? 'selected' : '' }}>Replacement</option>
                            <option value="Additional" {{ old('request_type', $supplyRequest->request_type) == 'Additional' ? 'selected' : '' }}>Additional</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-gray-100 pt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="md:col-span-2">
                        ...
                        <select name="category" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 @error('category') border-red-500 @enderror">
                            @foreach(['Office Supplies', 'School Supplies', 'Furniture', 'Equipment', 'IT Equipment', 'Cleaning Materials', 'Tools', 'Other'] as $cat)
                                <option value="{{ $cat }}" {{ old('category', $supplyRequest->category) == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="item_name" value="{{ old('item_name', $supplyRequest->item_name) }}"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="request_description" rows="2"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">{{ old('request_description', $supplyRequest->request_description) }}</textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Brand</label>
                        <input type="text" name="brand" value="{{ old('brand', $supplyRequest->brand) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="color" value="{{ old('color', $supplyRequest->color) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Size</label>
                        <input type="text" name="size" value="{{ old('size', $supplyRequest->size) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Type</label>
                        <input type="text" name="type" value="{{ old('type', $supplyRequest->type) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Quantity <span class="text-red-500">*</span></label>
                        <input type="number" name="quantity_requested" value="{{ old('quantity_requested', $supplyRequest->quantity_requested) }}" min="1"
                    ...
                    <div>
                        ...
                        <select name="unit" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 @error('unit') border-red-500 @enderror">
                            @foreach(['pcs', 'box', 'pack', 'ream', 'set', 'unit', 'liter', 'kg', 'meter', 'pair', 'bottle', 'roll'] as $u)
                                <option value="{{ $u }}" {{ old('unit', $supplyRequest->unit) == $u ? 'selected' : '' }}>{{ $u }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" step="0.01" name="unit_cost" value="{{ old('unit_cost', $supplyRequest->unit_cost) }}" min="0"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-sm">PHP</span>
                            <input type="number" step="0.01" name="total_cost" value="{{ old('total_cost', $supplyRequest->total_cost) }}" min="0"
                                   class="w-full pl-12 pr-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="semester" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                            ...
                            <option value="1st Semester" {{ old('semester', $supplyRequest->semester) == '1st Semester' ? 'selected' : '' }}>1st Semester</option>
                            <option value="2nd Semester" {{ old('semester', $supplyRequest->semester) == '2nd Semester' ? 'selected' : '' }}>2nd Semester</option>
                            <option value="Summer" {{ old('semester', $supplyRequest->semester) == 'Summer' ? 'selected' : '' }}>Summer</option>
                        </select>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">School Year</label>
                        <input type="text" name="school_year" value="{{ old('school_year', $supplyRequest->school_year) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-gray-100 pt-6">
                ...
                <textarea name="remarks" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">{{ old('remarks', $supplyRequest->remarks) }}</textarea>
            </div>
            ...
            <div class="border-t border-gray-100 pt-6 flex items-center justify-end space-x-3">
                ...
                <button type="submit"
                    ...
                    <i class="fa-solid fa-save mr-2"></i> Update Request
                </button>
            </div>
        </form>
    </div>
